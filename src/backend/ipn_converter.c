...
bool isUnaryMinus(const char *expression, int index) {
  ...
  if (expression[index] == '-' && index == 0) flag = true;
  // если унарный минус после открывающей скобки или оператора
  if (expression[index] == '-' &&
      (isOperator(expression[index - 1]) || expression[index - 1] == '('))
    flag = true;
  return flag;
}
...
bool isX(const char input) { return input == 'x' || input == 'X'; }

bool isParenthesis(const char input) { return input == '(' || input == ')'; }

bool isAlpha(const char input) {
  return (input >= 'a' && input <= 'z') || (input >= 'A' && input <= 'Z');
}

bool isDigit(const char input) { return (input >= '0' && input <= '9'); }
...
bool calculatePostfixForGraph(char *originalPostfix, double *result, double x) {
  ...
  if (flag == true) *result = popCalcStack(&stack);
